...
- `JIEZHU_ENABLE_JIEZHU_ABLITY` (default `OFF`): Whether to enable full ability of `jiezhu` (if `OFF`, only the core API is built, without functions like `chat_completions_jiezhu`)
